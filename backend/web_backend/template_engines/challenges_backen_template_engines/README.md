...
Build a small event listing site called **Event Hub**. Each step builds on the previous — work through them in order.

### Setup

Create a new Express + TypeScript project and install the required packages:
...
Create `views/base.html` with a `title` block and a `content` block. The base template should include:
- a header with the site name "Event Hub"
- a nav with links to Home and Events
- a footer
...
Implement the contact page. The form does not need to process submissions — plain HTML is sufficient. Use a macro for the form fields to avoid repeating input markup. The input should accept a `label`, `placeholder`, and `type` parameter. If the `type` is `"textarea"`, a `<textarea>` element should be rendered instead of an `<input>`.
